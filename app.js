...
function strokeRing(ctx, x, y, r) {
  ...
  ctx.strokeStyle = "rgba(90, 68, 32, 0.55)";
  ctx.lineWidth = 2.5;
  ctx.stroke();
}
...
function circleSticker(img) {
  ...
  const srcY = img.height > img.width ? (img.height - side) * 0.12 : (img.height - side) / 2;
  ctx.drawImage(img, (img.width - side) / 2, srcY, side, side, 0, 0, 512, 512);
  ctx.restore();
  strokeRing(ctx, 256, 256, 240);
  const texture = new THREE.CanvasTexture(canvas);
  texture.colorSpace = THREE.SRGBColorSpace;
  return texture;
}
...
void main() {
  ...
  vec2 uv = vec2(dot(n, T), dot(n, B)) / (0.72 + 0.28 * facing);
  float r = length(uv);
  vec2 texUV = uv * 0.5 + 0.5;
  float mask = smoothstep(1.05, 0.78, r) * smoothstep(0.02, 0.18, facing);
  vec4 port = texture2D(portrait, texUV);
  vec3 keyL = normalize(vec3(0.35, 0.78, 0.42));
  float ndKey = max(dot(n, keyL), 0.0);
  float hemi = n.y * 0.5 + 0.5;
  vec3 lit = ballColor * (0.28 + 0.42 * hemi + 0.55 * ndKey);
  vec3 h = normalize(V + keyL);
  float spec = pow(max(dot(n, h), 0.0), 64.0);
  float curve = mix(1.0, 0.78, smoothstep(0.15, 1.0, r));
  vec3 face = port.rgb * (0.78 + 0.22 * ndKey) * curve;
  vec3 col = mix(lit, face, mask * port.a);
  col += vec3(0.95, 0.92, 0.82) * spec * mix(0.22, 0.04, mask);
  col += vec3(0.82, 0.72, 0.42) * selected * 0.16;
...
function paintName(name) {
  ...
  ctx.lineWidth = 5;
  ctx.strokeStyle = "rgba(6, 16, 24, 0.85)";
...
scene.add(new THREE.HemisphereLight(0xb7c8d8, 0x081018, 0.55));
const key = new THREE.DirectionalLight(0xfff1dc, 1.15);
key.position.set(10, 16, 12);
key.castShadow = true;
key.shadow.mapSize.set(1024, 1024);
scene.add(key);
const fill = new THREE.DirectionalLight(0x6e90b0, 0.35);
...
function tick() {
  ...
  for (const mesh of nodes) {
    ...
    if (!reducedMotion) {
      mesh.position.y = base.y + Math.sin(t * 0.72 + phase) * 0.11;
    }
    nameSprite.position.copy(mesh.position);
    nameSprite.position.y -= radius + 0.62;
  }
  writeLinks();
  controls.update();
...
async function buildPeople() {
  ...
  PEOPLE.forEach((person, i) => {
    ...
    const color = new THREE.Color(0x1a2c3c);
    ...
    const nameW = Math.max(1.55, person.name.length * 0.42);
    nameSprite.scale.set(nameW, nameW * 0.26, 1);
